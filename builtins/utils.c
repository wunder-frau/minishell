...
void ft_cd(t_minishell *shell, char **argv)
{
	...
	shell->pwd = malloc(sizeof(char) * (ft_strlen(pwd) + 1));
	ft_strlcpy(shell->pwd, pwd, ft_strlen(pwd) + 1);
	printf("ft_cd.shell->pwd: %s\n", shell->pwd);
    ...
    // Debug prints
    printf("IN FT_CD END shell->hashmap PWD w ft_get_env: %s\n", ft_get_env(*(shell->hashmap), "PWD"));
}
